#!/bin/bash
# scripts/run_arm_tests.sh
# Fire & Forget robust test rig for py-flow
# 
# NOTE: This script is designed specifically for the LINUX ARM case, where 
# there is more limited memory and Docker processes to manage.
#
# Usage (run in background and detach from terminal):
#   nohup ./scripts/run_arm_tests.sh > /dev/null 2>&1 &
set -u
...
LOG_FILE="${LOG_DIR}/${TIMESTAMP}_arm_details.txt"
SUMMARY_FILE="${LOG_DIR}/${TIMESTAMP}_arm_summary.txt"
