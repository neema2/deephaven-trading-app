...
def _load_pgserver():
    """Surgical architecture detection and package selection."""
    # 1. Always prioritize the standard package (macOS ARM/x86 and Windows/x86)
    try:
        import pgserver
    ...
    # 2. Narrow fallback for Linux ARM64 (e.g. WSL on ARM)
    # This is specifically where the standard pgserver package often lacks wheels.
    if platform.system() == "Linux" and platform.machine() in ("aarch64", "arm64"):
        try:
            import pixeltable_pgserver as pixeltable
            logger.info("Using pixeltable-pgserver for Linux/ARM compatibility")
            return pixeltable, pixeltable.__file__
        except ImportError:
            pass

    return None, None

# Initialize singleton-style exports
_mod, PGSERVER_FILE = _load_pgserver()
PostgresServer = getattr(_mod, 'PostgresServer', None)
pgserver = _mod
...
def ensure_uuid_ossp_shim(pgserver_module_file):
    ...
    try:
        ...
        os.chmod(control, 0o644)  # Ensure world-readable for pgserver user
        ...
        os.chmod(sql, 0o644)  # Ensure world-readable for pgserver user
...
def ensure_pgcrypto_shim(pgserver_module_file):
    """
    Ensure a dummy pgcrypto SQL shim exists in the PostgreSQL extension directory.
    Lakehouse migration requires this extension to be present if the DB instance 
    is minimal (like pixeltable-pgserver on ARM WSL).
    """
    if not pgserver_module_file:
        return
