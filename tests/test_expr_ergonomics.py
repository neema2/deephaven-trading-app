...
float arithmetic while transparently building a correct, flat Expr DAG:
  ...
  4. Scalar early-return from @computed_expr  — returning a bare 0.0 from a
     @computed_expr body is legal; get_expr() auto-wraps it via _wrap().
...
# ── 4. Scalar early-return from @computed_expr ────────────────────────────────
...
class TestComputedExprScalarReturn:
    """@computed_expr bodies may return plain scalars; get_expr() wraps them."""

    def _make_dummy(self, return_value):
        """Build a class with a @computed_expr method returning return_value."""
    ...
    def test_get_expr_wraps_float(self):
        """get_expr() should wrap a plain float return as Const."""
        from reactive.expr import Const as C
        _Dummy = self._make_dummy(0.0)
        prop = _Dummy.__dict__["prop"]
        result = prop.get_expr(_Dummy())
