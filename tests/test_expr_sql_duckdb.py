...
from reactive.expr import diff, eval_cached, Expr
...
def expr_to_executable_sql(expr: Expr, ctx: dict[str, float]) -> str:
    ...
    This is used for testing individual Expr fragments (like NPV) against
    Python evaluation in DuckDB.
    """
...
def portfolio(curve):
    p = Portfolio()
    p.add_instrument("SWAP1", IRSwapFixedFloatApprox(
        notional=10e6, fixed_rate=0.03, tenor_years=3.0, curve=curve,
    ))
    p.add_instrument("SWAP2", IRSwapFixedFloatApprox(
        notional=10e7, fixed_rate=0.04, tenor_years=7.0, curve=curve,
    ))
    return p
...
class TestPortfolioPythonVsSQL:
    ...
    def portfolio(self, curve):
        p = Portfolio()
        p.add_instrument("3Y", IRSwapFixedFloatApprox(
            notional=30_000_000, fixed_rate=0.04, tenor_years=3.0, curve=curve,
        ))
        p.add_instrument("5Y", IRSwapFixedFloatApprox(
            notional=50_000_000, fixed_rate=0.05, tenor_years=5.0, curve=curve,
        ))
        p.add_instrument("10Y", IRSwapFixedFloatApprox(
...
class TestOptimizedPortfolioSQL:
    ...
    def test_optimized_sql_is_pruned(self, curve):
        ...
        p.add_instrument("SHORT", IRSwapFixedFloatApprox(
            notional=10e6, fixed_rate=0.03, tenor_years=3.0, curve=curve,
        ))
