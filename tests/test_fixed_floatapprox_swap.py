...
@pytest.fixture
def curve():
    """A LinearTermDiscountCurve with three pillar points."""
    points = [
...
class TestPortfolio:
    ...
    def portfolio(self, curve):
        p = Portfolio()
        p.add_instrument("3Y", IRSwapFixedFloatApprox(symbol="PORT_3Y", notional=30_000_000, fixed_rate=0.04, tenor_years=3.0, curve=curve))
    ...
    def test_npv_near_zero_at_par(self, portfolio, ctx):
        ...
        for name, npv in expr_npvs.items():
            notional = portfolio._instruments[name].notional
    ...
    def test_jacobian_shape(self, portfolio, ctx):
        ...
        jac = portfolio.eval_instrument_risk(ctx)
...
class TestExprFitter:
    ...
    def test_solve_npv_near_zero(self):
        ...
        curve = LinearTermDiscountCurve(name="CURVE_NPV", points=[p1, p5, p10])
