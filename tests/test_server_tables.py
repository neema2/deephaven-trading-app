...
using the DH Python API directly (same process as JVM).
...
# 1. Create writers
prices_writer = DynamicTableWriter({
...
# 2. Derive tables
prices_raw = prices_writer.table
...
import deephaven.execution_context as ec
scope = ec.get_exec_ctx().j_exec_ctx.getQueryScope()
scope.putParam("prices_raw", prices_raw.j_table)
scope.putParam("prices_live", prices_live.j_table)
scope.putParam("risk_raw", risk_raw.j_table)
scope.putParam("risk_live", risk_live.j_table)
scope.putParam("portfolio_summary", portfolio_summary.j_table)
scope.putParam("top_movers", top_movers.j_table)
scope.putParam("volume_leaders", volume_leaders.j_table)

# 3. Background ticker loop (runs on server)
...
def _ticker():
    ...
    def write_tick():
        for sym in _SYMBOLS:
            ...
            prices_writer.write_row(sym, p, p - 0.1, p + 0.1,
                                    int(500000 + random.random() * 1e6), c, c / p * 100)
            pos = random.randint(100, 1000)
            risk_writer.write_row(sym, p, pos, p * pos, c * pos,
...
def _publish_tables(client: StreamingClient = None):
    """Create TickingTables, derive all 7 tables, start background ticker via script."""
    if not client:
        raise RuntimeError("StreamingClient required for table setup")
